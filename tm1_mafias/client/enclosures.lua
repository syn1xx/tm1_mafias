...
if donationsfinish == true then
	for i,c in pairs(prices) do
		TriggerServerEvent("504_mafias:print",c)
		c = c * 0.5
	end
end
function getShop()
...
AddEventHandler('playerSpawned', function(spawn)
	...
	TriggerServerEvent('504_mafias:enclosureComprobate',enclosuresa)
end)
local enclosuresa = {}
	for _, info in pairs(enclosures) do
	    enclosuresa[_] = info.name      	
...
RegisterNetEvent('504_mafias:puttedGroupsInEnclosures')
AddEventHandler('504_mafias:puttedGroupsInEnclosures',function()
	Citizen.CreateThread(function()
		local enclosuresa = {}
		for _, info in pairs(enclosures) do
	      	enclosuresa[_] = info.name      	
...
--===========
RegisterNetEvent('504_mafias:putGroupsInEnclosures')
AddEventHandler('504_mafias:putGroupsInEnclosures',function(enclosuresInfo)
...
RegisterNetEvent('504_mafias:changeIsPosible')
AddEventHandler('504_mafias:changeIsPosible',function(isPosiblea)
	isPosible = isPosiblea
end)

RegisterNetEvent('504_mafias:activeAttack')
AddEventHandler('504_mafias:activeAttack',function(x1,x2,x3,name,gr,gr1,groupToGroup)
	if groupToGroup then
		backOperation(5,"attackWithIn1",name,x1,x2,x3,gr,gr1)
	else
		backOperation(5,"attackWithIn",name,x1,x2,x3,gr,gr1)
	end
	TriggerServerEvent('504_mafias:advert',"Has empezado el ataque, ten cuidado y no te alejes mas de 15 metros.")
	TriggerServerEvent('504_mafias:refreshIsPosible',false)
	state = false
end)
--===========
-- BORRAR VEHÍCULO
--===========
RegisterNetEvent('504_mafias:clearV')
AddEventHandler('504_mafias:clearV',function(x,y,z)
				...
				for i,v in pairs(enclosures) do
					if v.name == group.enclosure then
						...
						elseif GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), v.vestPoint.x,v.vestPoint.y,v.vestPoint.z, true) < 1.5 then
							...
							if IsControlJustReleased(0, 38) then
								if v.vest then
									ESX.TriggerServerCallback('504_mafias:hasEnoughMoney', function(bool)
										if bool == true then
								...
								if v.owner == nil then
									if IsControlJustReleased(0, 38) then
										if isPosible then
											...
											TriggerServerEvent('504_mafias:advert',"No es posible hacer el robo ahora mismo")
										end
									end
								else
									if IsControlJustReleased(0, 38) then
										if isPosible then
											TriggerServerEvent('504_mafias:getZoneWithin',v.owner,v.attack.x,v.attack.y,v.attack.z,v.name,group.groupid,true)
											TriggerServerEvent('504_mafias:advert',"Procesando información, no toques nada")
										else
											TriggerServerEvent('504_mafias:advert',"No es posible hacer el robo ahora mismo")
										end
									end
							...
							if GetDistanceBetweenCoords(coords, v.attack.x,v.attack.y,v.attack.z, true) < 1.5 then
								...
								if IsControlJustReleased(0, 38) then
									if v.owner == nil then
										...
										if isPosible then
											TriggerServerEvent('504_mafias:getZoneWithin',v.owner,v.attack.x,v.attack.y,v.attack.z,v.name,group.groupid)
											TriggerServerEvent('504_mafias:advert',"Procesando información, no toques nada")
										else
											TriggerServerEvent('504_mafias:advert',"No es posible hacer el robo ahora mismo")
										end
...
function backOperation(s,operation,encl,x,y,z,gr,gr1)
	...
	while true do
		...
		if isIn == false then
			TriggerServerEvent('504_mafias:advert',"Se ha suspendido el ataque por alejarte demasiado")
			state = 1
			break
		end
		if IsEntityDead(GetPlayerPed(-1)) then
			TriggerServerEvent('504_mafias:advert',"Ataque suspendido.")
			state = 1
			break
		end
		if seconds == 0 then
			if operation == "attackWithout" then
				state = 1
				TriggerServerEvent('504_mafias:advert',"La zona ya es tuya")
				TriggerServerEvent('504_mafias:getZoneFinished',encl,group.groupid,operation)
				Citizen.Wait(1000)
				TriggerServerEvent('504_mafias:refreshIsPosible',true)
			...
			elseif operation == "attackWithIn" then
				state = 1
				TriggerServerEvent('504_mafias:advert',"La zona ya es tuya")
				TriggerServerEvent('504_mafias:getZoneFinished',encl,gr1,operation,gr)
				Citizen.Wait(1000)
				TriggerServerEvent('504_mafias:refreshIsPosible',true)
	...
	ESX.UI.Menu.Open(
		...
		function(data, menu)
			if data.current.whatItIs == "weapon" then
				TriggerServerEvent('504_mafias:buyWeapon',data.current.value,data.current.money)
...
RegisterNetEvent('504_mafias:esposar')
AddEventHandler('504_mafias:esposar',function(option)
	local player, distance = ESX.Game.GetClosestPlayer()
	if distance ~= -1 and distance <= 3.0 then	
		TriggerServerEvent('504_mafias:toClientEsposar',GetPlayerServerId(player),option)
	end
end)

RegisterNetEvent('504_mafias:changeEsposar')
AddEventHandler('504_mafias:changeEsposar',function(option)
  IsHandcuffed    = option
...
function OpenPutStocksMenu(enclosure)
	ESX.TriggerServerCallback('504_mafias:getPlayerInventory', function(inventory)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_put_item_count', {
				...
				if count == nil then
					...
					menu2.close()
					menu.close()
					TriggerServerEvent('504_mafias:putStockItems', itemName, count, enclosure.society)
...
function OpenGetStocksMenu(enclosure)
	ESX.TriggerServerCallback('504_mafias:getMafiasInventory', function(items)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_get_item_count', {
				...
				if count == nil then
					...
					menu2.close()
					menu.close()
					TriggerServerEvent('504_mafias:getStockItem', itemName, count, enclosure.society)
